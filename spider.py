...
from result_generator import generator
...
class Spider(object):
    def __init__(self, config):
        ...
        if type(self.config['user_id_list']) == type(u""):
            ...
            with open(self.config['user_id_list'], 'rb') as f:
                ...
                self.config['city_list'] = []
                for line in lines:
                    temp = line.split(" ")
                    if len(temp) > 0 and line.split(' ')[0].isdigit() and len(temp[1]) > 0:
                        self.config['city_list'].append(temp[1])
                    else:
                        self.config['city_list'].append("未知城市")
                ...
                if weibo:
                    ...
                    if publish_time < since_date:
                        if self.parser.is_pinned_weibo(info[i]):
                            ...
                            return True
                    if self.date == "":
                        self.date = publish_time
                    print(publish_time)
                    self.weibo.append(weibo)
                    self.weibo_id_list.append(weibo['id'])
                    print(self.date)
                    if self.date != publish_time:
                        result = generator(self.weibo_list_today,self.user['nickname'],self.city_name,self.date.strftime("%m/%d/%Y"))
                        self.date = publish_time
                        self.weibo_list_today = []
                        print(result)
                        self.writer.write_weibo(result)

                    self.got_num += 1
                    self.weibo_list_today.append(weibo)
    ...
    def get_weibo_info(self):
        ...
        for page in tqdm(range(1, page_num + 1), desc='Progress'):
            ...
            if is_end:
                result = generator(self.weibo_list_today, self.user['nickname'], self.city_name,
    ...
    def start(self):
        ...
        for user_id in self.config['user_id_list']:
            self.city_name = self.config['city_list'][index]
